...
router.post('/booktable',async(req,res)=>{
    if(req.session.user_id){
        ...
        var sql=`insert into booking(id,name,email,booking_date,booking_time,no) values(?,?,?,?,?,?)`;

        var values=[req.session.user_id,booking_name,booking_email,booking_date,booking_time,booking_no];
...
router.post('/savecontact',async(req,res)=>{
    if(req.session.user_id)
    {
         ...
         var values=[name,email,subject,msg];
         await exe(sql,values);
         res.redirect('/');
    }
    else{
